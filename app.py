...
def editorTest():
	if validate(session):
		...
		userCode = content.find_one({'owner':userData['_id'], 'type':'editor_standalone'})
		if userCode == None:
			linkID = make_linkID()
			content.insert_one({
				'_id':str(uuid.uuid4()),
...
def contentSet():
	...
	if contentBefore['type'] == 'challenge':
		if 'instructions' in setRequest and owner:
			setRequest['instructions'] = setRequest['instructions'].replace("<safe>", "")
			setRequest['instructions'] = setRequest['instructions'].replace("</safe>", "")
...
def newChallenge():
	if validate(session):
		userData = users.find_one({'username': session['username']})
		userCode = content.find_one({'owner':userData['_id'], 'type':'challenge'})
		if userCode == None:
			linkID = make_linkID()
			content.insert_one({
				'_id':str(uuid.uuid4()),
...
def setCode():
	if validate(session):
		userData = users.find_one({'username': session['username']})
		userCode = codes.update_one({'owner':userData['_id']}, {'$set':{'content':request.json['code']}})
...
def runCode():
	...
	if response.overallResultType == reqres_pb2.Response.RunResultType.CompilerError:
		try:
			...
			formattedError = formattedError[formattedError.index(SEARCH)+len(SEARCH)+4:]
		except:
			formattedError = response.results[0].methodOutput.replace("\\n", "<br>").replace("\\r", "")
...
def getContent(linkID):
	...
	if validate(session):
		...
		if contentData['owner'] == userData['_id']:
			...
			editorChallenge = content.find_one({'owner':userData['_id'], 'type':'editor_challenge', 'assocChallenge':contentData['_id']})
			if not editorChallenge:
				contentID = str(uuid.uuid4())
				args_immutable = [{'id':arg['id'], 'arg':arg['arg'], 'match':False} for arg in contentData['args_mutable']]
				content.insert_one({
					'_id':contentID,
